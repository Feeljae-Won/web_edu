...
public class MemberDAO extends DAO{
	...
	public int delete(MemberVO vo) throws Exception {
		...
		try {
			...
			System.out.println("상태 변경 완료");
				...
				e.printStackTrace();
				// 특별한 예외는 그냥 전달한다.
				if(result == 0); System.out.println("예외 발생 : 아이디나 비밀번호가 맞지 않습니다. 다시 확인해 주세요. "); // 오류 나면 던진다
				// 그 외 처리 중 나타나는 오류에 대해서 사용자가 볼 수 잇는 예외로 만들어 전달한다.
	...
	public LoginVO login(LoginVO loginvo) throws Exception {
		...
		try {
			...
			if (vo == null) { // ID와 PW가 맞지 않는다. -> 예외로 처리
				...
				if(e.getMessage().indexOf("예외 발생") >= 0) throw e; // 오류 나면 던진다
				// 그 외 처리 중 나타나는 오류에 대해서 사용자가 볼 수 잇는 예외로 만들어 전달한다.
				else throw new Exception("예외 발생 : 로그인 처리 중 예외가 발생했습니다.");
			} finally {
				// 7. 닫기
				DB.close(con, pstmt, rs);
			} // end of try ~ finally
		...
		public int conUpdate(LoginVO vo) throws Exception {
			...
			int result = 0;
				...
				try {
					...
					if(e.getMessage().indexOf("예외 발생") >= 0) throw e; // 오류 나면 던진다
					// 그 외 처리 중 나타나는 오류에 대해서 사용자가 볼 수 잇는 예외로 만들어 전달한다.
					else throw new Exception("예외 발생 : 최근 접속일 DB 처리 중 예외가 발생했습니다.");
				} finally {
					// 7. 닫기
					DB.close(con, pstmt);
				} // end of try ~ finally
	...
	final String UPDATE = "update member set "
			+ " name =  ?, gender = ?, birth = ?, tel = ?, email = ?, photo = ?"
			+ " where id = ? and pw = ?";
